...
const DEFAULT_DTS_PATH = '.auto-exhibition-template-sdk/auto-exhibition-template-sdk.generated.d.ts'
const CONFIG_JSON_FILE_NAME = 'config.json'
const VALUE_MAP_FILE_NAME = 'assets/auto-exhibition-template-sdk/valueMap.json'
const INDENT = '  '
...
// Generated by auto-exhibition-template-sdk/vite from ${relativeConfigPath}.
// Do not edit this file manually.

type TemplateProjectValueMap = ${valueMapType}

declare module 'auto-exhibition-template-sdk' {
...
export function templateSdkPlugin(options: TemplateSdkPluginOptions): Plugin {
  if (!options || options.configJson === undefined) {
    throw new Error('auto-exhibition-template-sdk/vite 需要显式传入 configJson 对象，例如 templateSdkPlugin({ configJson })')
  }
  ...
  const generateTypes = async (reportError: (message: string) => void) => {
    try {
      ...
      reportError(`auto-exhibition-template-sdk/vite 生成类型声明失败:\n${message}`)
    }
  }

  return {
    name: 'auto-exhibition-template-sdk:vite',
    enforce: 'pre',
    ...
    generateBundle() {
      try {
        ...
        // config.json 固定放在 dist 根目录，便于平台解析模板包的 schema 入口；
        // valueMap 保持在 assets/auto-exhibition-template-sdk 下，继续作为模板运行时可直接访问的値快照。
        // dev 阶段仍只生成类型声明，避免开发时频繁写入 public 或 dist 目录。
        emitTemplateJsonFiles((emittedFile) => this.emitFile(emittedFile), options)
      } catch (error) {
        const message = error instanceof Error ? error.stack || error.message : String(error)
        this.error(`auto-exhibition-template-sdk/vite 生成模板 JSON 产物失败:\n${message}`)
      }
    },
  }
}
