...
1. `auto-exhibition-template-sdk`：默认导出 `TemplateSdk`，并导出 `useTemplateValue`、`validateTemplateConfig`、`buildTemplateValueMap`、`buildTemplateArtifacts`、`buildTemplateJsonFiles`。
2. `auto-exhibition-template-sdk/config`：导出 `defineTemplateConfig`。
3. `auto-exhibition-template-sdk/vite`：导出 `templateSdkPlugin`。
...
2. 插件生成 `.auto-exhibition-template-sdk/**/*.d.ts`，并产出 `config.json` 和 `valueMap.json`。
3. 模板项目运行时只安装 `TemplateSdk`。
4. `TemplateSdk` 自动读取 `./config.json` 和 `./assets/auto-exhibition-template-sdk/valueMap.json`。
...
3. `src/runtime/schema`：`configJson` 校验、`file` 类型内置文件类型 JSON、构建产物生成和 JSON 导出入口。
...
SDK 采用"源码目录 + dist 分发目录"的结构：

1. 开发时维护 `src`。
2. 发布前执行 `pnpm build`，构建后产物输出到 `dist`。
3. 模板项目安装时消费的是包根兼容入口，再转到 `dist`。
4. 发布新版本前，更新 `package.json` 中的 `version` 字段，再执行 `npm publish`。
